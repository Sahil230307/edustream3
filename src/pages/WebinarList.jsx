...
export default function WebinarList({ webinars, user }) {
  ...
  const difficulties = ["All", ...new Set(webinars.map(w => w.difficulty))];
  ...
  const filtered = useMemo(() => {
    return webinars.filter(w => {
      ...
      const matchDifficulty = selectedDifficulty === "All" || w.difficulty === selectedDifficulty;
            ...
            {paginatedWebinars.map(webinar => (
              <div key={webinar.id} className="webinar-card">
                <div className="card-image">
                  ...
                  <button 
                    className={`wishlist-btn ${wishlist.includes(webinar.id) ? 'active' : ''}`}
                    onClick={() => handleAddToWishlist(webinar.id)}
                    title={wishlist.includes(webinar.id) ? "Remove from wishlist" : "Add to wishlist"}
                  >
                ...
                <div className="card-content">
                  ...
                  <div className="card-capacity">
                    <span className="capacity-text">{webinar.registeredCount}/{webinar.maxCapacity} Registered</span>
                  ...
                  <div className="card-rating">
                    <span className="stars">{'⭐'.repeat(Math.round(webinar.ratings))}</span>
                    <span className="rating-value">{webinar.ratings}/5</span>
                  </div>
